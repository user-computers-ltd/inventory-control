<?php
  ...
  if (assigned($debtorCode) && assigned($currencyCode) && assigned($exchangeRate) && assigned($discount) && assigned($tax) && assigned($warehouseCode) && assigned($soNos) && assigned($brandCodes) && assigned($modelNos) && assigned($prices) && assigned($qtys)) {
    $piNo = "INV" . date("YmdHis");
    $date = date("Y-m-d");

    query("
      INSERT INTO
        `pi_header`
          (pi_no, pi_date, debtor_code, currency_code, exchange_rate, discount, tax, warehouse_code)
        VALUES
          (\"$piNo\", \"$date\", \"$debtorCode\", \"$currencyCode\", \"$exchangeRate\", \"$discount\", \"$tax\", \"$warehouseCode\")
    ");
    ...
    for ($i = 0; $i < count($soNos); $i++) {
      ...
      array_push($values, "(\"$piNo\", \"$i\", \"$soNo\", \"$brandCode\", \"$modelNo\", \"$price\", \"$qty\")");
    }

    query("
      INSERT INTO
        `pi_model`
          (pi_no, pi_index, so_no, brand_code, model_no, price, qty)
          VALUES
      " . join(", ", $values));

    header("Location: ../invoice.php?pi_no=$piNo");
  }
